defmodule XoWeb.UserAuthTest do
  use XoWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Xo.Accounts
  alias Xo.Accounts.Scope
  alias XoWeb.UserAuth

  import Xo.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, XoWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      XoWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        XoWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
